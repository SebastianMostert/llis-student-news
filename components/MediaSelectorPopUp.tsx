...
import {
    ...
    TransformedImage,
    checkImageUsage,
} from "@/actions/media"; // Add deleteImage to handle server deletion
...
const MediaSelectorPopUp = ({
    ...
    useEffect(() => {
        const fetchMediaFiles = async () => {
            try {
                ...
                toast.error(t("errorFetchingMedia"));
        ...
        if (file) {
            ...
            try {
                ...
                if (compressedFile.size > FILE_MAX_SIZE) {
                    toast.error(t("compressedFileTooLarge"));
                    return;
                }
                ...
                onSelect(
                    uploadedImage.id,
                    `data:${uploadedImage.mimeType};base64,${uploadedImage.content}`
                );
    ...
    const deleteImageFromState = async (id: string) => {
        // Remove file on the server
        await deleteImage(id);

        // Update the local state to remove the image
        setMediaFiles((prevMediaFiles) =>
            prevMediaFiles.filter((media) => media.id !== id)
        );
    ...
    const handleDelete = async (id: string) => {
        try {
            ...
            toast.error(t("errorDeletingMedia"));
            ...
            <div className="bg-primaryBg-light dark:bg-primaryBg-dark rounded-lg shadow-lg w-3/4 max-w-lg p-6">
                ...
                <div className="grid grid-cols-3 gap-4">
                    ...
                    {mediaFiles.map(({ id, mimeType, content }, index) => (
                        <div
                            key={id}
                            className={`relative cursor-pointer border rounded-md overflow-hidden hover:shadow-lg ${`data:${mimeType};base64,${content}` === selected
                                ? "border-blue-500"
                                : "border-none"
                                }`}
                            onClick={() =>
                                onSelect(id, `data:${mimeType};base64,${content}`)
                            }
                            onContextMenu={(e) => {
                                e.preventDefault();
